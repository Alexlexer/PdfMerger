...
fn build_synthesis_prompt(
    ...
    let task = if intermediate {
        ...
        "Produce the final document summary in at most 10 concise bullets and finish the answer within the available space. Keep separate documents separate and include at least one bullet for every distinct non-log document. Prioritize official decisions and their stated reasons, certified periods, important dates, monetary totals, and obligations over contact or reference details. Compress every repetitive call-log section into one combined bullet; never enumerate log pages, dates, or phone numbers. Copy dates and amounts exactly; never infer or alter them. Omit personal names, addresses, identifiers, control codes, and boilerplate unless essential. Do not treat control codes as organizations and do not invent agreements."
    };
    format!(
